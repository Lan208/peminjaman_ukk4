...
<div class="h-screen bg-gray-50 overflow-hidden">
    <div class="h-full flex flex-col p-4 sm:p-6 overflow-hidden">
        ...
        <!-- HEADER -->
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4 pb-3 border-b border-gray-100 flex-shrink-0">
            ...
            <div class="overflow-x-auto h-full">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <div id="pinjamModal{{ $b->id }}" class="hidden fixed inset-0 bg-black/30 flex justify-center items-center">
                            <div class="bg-white p-4 rounded w-80">
                                <form action="/pinjam/{{ $b->id }}" method="POST">
                                    ...
                                    <input type="number" name="jumlah" max="{{ $b->stok }}" class="w-full mb-2 border p-2">
                        ...
                        @auth
                        @if(auth()->user()->role == 'admin')
                        <div id="editModal{{ $b->id }}" class="hidden fixed inset-0 bg-black/30 flex justify-center items-center">
                            <div class="bg-white p-4 rounded w-80">
                                <form method="POST" action="/books/{{ $b->id }}">
                                    @csrf
                                    @method('PUT')
                                    <input type="text" name="judul" value="{{ $b->judul }}" class="w-full mb-2 border p-2">
                                    <input type="text" name="penulis" value="{{ $b->penulis }}" class="w-full mb-2 border p-2">
                                    <input type="text" name="penerbit" value="{{ $b->penerbit }}" class="w-full mb-2 border p-2">
                                    <input type="number" name="tahun_terbit" value="{{ $b->tahun_terbit }}" class="w-full mb-2 border p-2">
                                    <input type="number" name="stok" value="{{ $b->stok }}" class="w-full mb-2 border p-2">

                                    <div class="flex justify-end gap-2">
                                        <button type="button" onclick="closeModal('editModal{{ $b->id }}')">Batal</button>
